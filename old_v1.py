...
def place_general_orders_buy_sell(symbol):
    ...
    if not buy_positions:
        ...
        if buy_price <= latest_buy_position_price - pips_200 * mt5.symbol_info(symbol).point:
    ...
    if not sell_positions:
        ...
        if sell_price >= latest_sell_position_price + pips_200 * mt5.symbol_info(symbol).point:
...
def martingale_strategy(symbol):
    ...
    initial_equity = get_current_equity()
    print(f"Main function - initial equity is: {initial_equity}")
