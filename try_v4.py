...
def close_orders_based_on_conditions(symbol, price_15m, upper_band_15m, lower_band_15m, middle_band_15m, allow_buy_orders, allow_sell_orders, m5_trend, m15_trend, h1_trend, signal, final_trend, direction):
...
# scenairo 1
    ...
    direc_sell_positions = [pos for pos in positions if pos.type == mt5.ORDER_TYPE_SELL and pos.magic == direc_sell_magic]
    if direc_sell_positions:
        if direction == "UP":
            print(f"closing all sell orders because all trend indicators are showing BUY")
            close_strategy_orders(symbol, direc_sell_magic)

    # if allow_sell_orders and not allow_buy_orders: # and m5_trend == "SELL":
    direc_buy_positions = [pos for pos in positions if pos.type == mt5.ORDER_TYPE_BUY and pos.magic == direc_buy_magic]
    if direc_buy_positions:
        if direction == "DOWN":
            print(f"closing all buy orders because all trend indicators are showing SELL")
            close_strategy_orders(symbol, direc_buy_magic)

    signal_sell_positions = [pos for pos in positions if pos.type == mt5.ORDER_TYPE_SELL and pos.magic == signal_sell_magic]
    if signal_sell_positions:
        if signal == "EXIT_SELL":
            print(f"closing all sell orders because all trend indicators are showing BUY")
            close_strategy_orders(symbol, signal_sell_magic)

    # if allow_sell_orders and not allow_buy_orders: # and m5_trend == "SELL":
    signal_buy_positions = [pos for pos in positions if pos.type == mt5.ORDER_TYPE_BUY and pos.magic == signal_buy_magic]
    if signal_buy_positions:
        if signal == "EXIT_BUY":
            print(f"closing all buy orders because all trend indicators are showing SELL")
            close_strategy_orders(symbol, signal_buy_magic)
...
# ========= GET MACD TRENDS DATA ========== 
    ...
    # print(f" final_trend: {final_trend}, M5: {m5_trend} ({m5_macd:.5f}), M15: {m15_trend} ({m15_macd:.5f}), H1: {h1_trend} ({h1_macd:.5f}), Rev: {h1_reversal}, Type: {reversal_type}, RSI: {rsi}, bb_gap: {bb_gap}, direc: {direction}, sig: {signal}")
    ...
    close_orders_based_on_conditions(symbol, price_15m, upper_band_15m, lower_band_15m, middle_band_15m, allow_buy_orders, allow_sell_orders, m5_trend, m15_trend, h1_trend, signal, final_trend, direction)
    ...
    if bb_gap < 80:
        if not martingale_buy_positions:    
            ...
            if buy_price <= lower_band_15m + 2:  # Initial buy condition
        ...
        if not martingale_sell_positions:
            ...
            if sell_price >= upper_band_15m - 2:  # Initial sell condition
    ...
    if not signal_buy_positions:    
        martingale_buy_order_qty = martingale_lot_sizes[0]
        if rsi < 30 and bb_gap > 30 and signal == "EXIT_SELL":  # Initial buy condition
            place_buy_order(symbol, martingale_buy_order_qty, signal_buy_magic)
            print(f"MACD Trends - final trend is: {final_trend} H1 Reversal: {h1_reversal}, Type: {reversal_type}, RSI: {rsi}, Supp: {latest['active_support']}, Res: {latest['active_resistance']}")
    if not signal_sell_positions:
        martingale_sell_order_qty = martingale_lot_sizes[0]
        if rsi > 70 and bb_gap > 30 and signal == "EXIT_BUY":  # Initial sell condition
