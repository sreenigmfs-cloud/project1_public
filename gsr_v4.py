...
def close_orders_based_on_conditions(symbol, buy_magic_num, sell_magic_num):
    global target_balance, my_balance
...
def is_trade_allowed(symbol, trade_type, timeframe=mt5.TIMEFRAME_M15, max_points=15):
    """
    ...
    up_move_points = (high_price - open_price) / point
    down_move_points = (open_price - low_price) / point
...
def place_general_orders_buy_sell(symbol):
    ...
    if not buy_positions and not sell_positions:
        my_balance = get_equity(symbol)

    if len(buy_positions) <= 5:
        diff_pips = 2500
    elif len(buy_positions) <= 10:
        diff_pips = 3500
    elif len(buy_positions) <= 15:
        diff_pips = 4500
    ...
    if len(sell_positions) <= 5:
        diff_pips = 2500
    elif len(sell_positions) <= 10:
        diff_pips = 3500
    elif len(sell_positions) <= 15:
        diff_pips = 4500
