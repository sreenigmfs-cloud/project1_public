...
# scenario 1 -- begins
    ...
    if current_equity >= target_balance:
        ...
        if len(buys) - len(sells) > 3 or len(sells) - len(buys) > 3:
            close_strategy_orders1(symbol)
...
def is_trade_allowed(symbol, trade_type, timeframe=mt5.TIMEFRAME_M30, max_points=15):
    """
    trade_type: 'buy' or 'sell'
    
    Returns:
        True  -> trade is allowed
        False -> trade should be blocked
    """

    rates = mt5.copy_rates_from_pos(symbol, timeframe, 0, 1)
    if rates is None or len(rates) == 0:
        return False  # safest: block trading
    ...
    # Movement in points from open
    up_move_points = (high_price - open_price)
    down_move_points = (open_price - low_price)
    # print(f"values are {open_price}, {high_price}, {low_price}, {point}, {up_move_points}, {down_move_points}")
    # 🚀 Strong bullish candle → block SELLs
    # 📉 Strong bearish candle → block BUYs

    if down_move_points >= max_points and trade_type.lower() == 'buy':
        return False

    if up_move_points >= max_points and trade_type.lower() == 'sell':
        return False
...
def place_general_orders_buy_sell(symbol):
    ...
    if len(buy_positions) < 3:
        diff_pips_buys = 3000
    elif len(buy_positions) < 6:
        diff_pips_buys = 4000
    elif len(buy_positions) < 9:
        diff_pips_buys = 5000
    elif len(buy_positions) < 20:
        diff_pips_buys = 6000

    if len(sell_positions) < 3:
        diff_pips_sells = 3000
    elif len(sell_positions) < 6:
        diff_pips_sells = 4000
    elif len(sell_positions) < 9:
        diff_pips_sells = 5000
    elif len(sell_positions) < 20:
        diff_pips_sells = 6000

    buy_order_qty = sell_order_qty = lot_sizes[0]

    # print(f"buy order - diff pips are - {diff_pips_buys} , {diff_pips_sells} buys - {len(buy_positions)} sells - {len(sell_positions)}")
    ...
    if not buy_positions:
        if is_trade_allowed(symbol, trade_type="buy"):        
            place_buy_order(symbol, buy_order_qty, buy_magic_num)
        elif len(sell_positions) - len(buy_positions) > 2:        
            place_buy_order(symbol, buy_order_qty, buy_magic_num)
    else:
        latest_buy_position_price = buy_positions[0].price_open

        if buy_price <= latest_buy_position_price - diff_pips_buys * mt5.symbol_info(symbol).point:
            if is_trade_allowed(symbol, trade_type="buy"):        
                place_buy_order(symbol, buy_order_qty, buy_magic_num)
            elif buy_price <= latest_buy_position_price - diff_pips_high * mt5.symbol_info(symbol).point:        
                place_buy_order(symbol, buy_order_qty, buy_magic_num)
            elif len(sell_positions) - len(buy_positions) > 2:        
                place_buy_order(symbol, buy_order_qty, buy_magic_num)

    if not sell_positions:
        if is_trade_allowed(symbol, trade_type="sell"):        
            place_sell_order(symbol, sell_order_qty, sell_magic_num)
        elif len(buy_positions) - len(sell_positions) > 2:        
            place_sell_order(symbol, sell_order_qty, sell_magic_num)
    else:
        latest_sell_position_price = sell_positions[0].price_open

        if sell_price >= latest_sell_position_price + diff_pips_sells * mt5.symbol_info(symbol).point:
            if is_trade_allowed(symbol, trade_type="sell"):        
                place_sell_order(symbol, sell_order_qty, sell_magic_num)
            elif sell_price >= latest_sell_position_price + diff_pips_high * mt5.symbol_info(symbol).point:        
                place_sell_order(symbol, sell_order_qty, sell_magic_num)
            elif len(buy_positions) - len(sell_positions) > 2:        
                place_sell_order(symbol, sell_order_qty, sell_magic_num)
