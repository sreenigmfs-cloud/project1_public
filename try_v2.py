...
tp_pips = 500
profit_target = 1000
martingale_lot_sizes = [0.01, 0.01, 0.01]
...
def close_orders_based_on_conditions(symbol, price_15m, upper_band_15m, lower_band_15m, middle_band_15m, allow_buy_orders, allow_sell_orders, m5_trend, m15_trend, h1_trend, signal):
...
# scenairo 1
    ...
    if martingale_sell_positions:
        if m5_trend == "BUY" and m15_trend == "BUY" and h1_trend == "BUY" or signal == "EXIT_SELL":
    ...
    if martingale_buy_positions:
        if m5_trend == "SELL" and m15_trend == "SELL" and h1_trend == "SELL" or signal == "EXIT_BUY":
...
# ========= GET MACD TRENDS DATA ========== 
    ...
    print(f"M5: {m5_trend} ({m5_macd:.5f}), M15: {m15_trend} ({m15_macd:.5f}), H1: {h1_trend} ({h1_macd:.5f}), Rev: {h1_reversal}, Type: {reversal_type}, RSI: {rsi}, buy: {buy_now_confirmed}, sell: {sell_now_confirmed}, bb_gap: {bb_gap}, dir: {direction}, signal:{signal}")
    ...
    close_orders_based_on_conditions(symbol, price_15m, upper_band_15m, lower_band_15m, middle_band_15m, allow_buy_orders, allow_sell_orders, m5_trend, m15_trend, h1_trend, signal)
    ...
    if not martingale_buy_positions:    
        ...
        if buy_price <= lower_band_15m:  # Initial buy condition
            if (m5_trend == "SELL" and m15_trend == "SELL" and h1_trend == "SELL") or direction == "DOWN" or rsi > sell_rsi or signal == "EXIT_BUY":
                pass
            else:
    ...
    if not martingale_sell_positions:
        ...
        if sell_price >= upper_band_15m:  # Initial sell condition
            if (m5_trend == "BUY" and m15_trend == "BUY" and h1_trend == "BUY") or  direction == "UP" or rsi < buy_rsi or signal == "EXIT_SELL":
                pass
            else:
