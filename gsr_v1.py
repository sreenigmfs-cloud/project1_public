...
"""
20 points and risky but in range good program
...
def place_buy_order(symbol, buy_order_qty, magic_num):
    ...
    tp = price + pips_profit * mt5.symbol_info(symbol).point
    ...
    if result is None:
        ...
        # if buy_order_qty == 0.02:
        #     if round(buy_total - sell_total, 2) > 0.05:
        #         place_sell_order(symbol, opposite_qty05, magic_num)
...
def place_sell_order(symbol, sell_order_qty, magic_num):
    ...
    if result is None:
        ...
        # if sell_order_qty == 0.02: 
        #     if round(sell_total - buy_total, 2) > 0.05:
        #         place_buy_order(symbol, opposite_qty05, magic_num)
...
# scenario
    buy_pos = [bpos for bpos in positions if bpos.type == mt5.ORDER_TYPE_BUY]
    sell_pos = [spos for spos in positions if spos.type == mt5.ORDER_TYPE_SELL]

    if total_positions > 2:
        if len(buy_pos) == len(sell_pos):
            close_strategy_orders1(symbol)
...
# scenario 1 -- end
    ...
    # # if not buy_pos:
    # #     close_strategy1_orders(symbol, buy_magic_num)
    # # if not sell_pos:
    # #     close_strategy1_orders(symbol, sell_magic_num)
    # # # ...................................... END

    # # Filter positions by magic numbers
    # buy_positions = [pos for pos in positions if pos.magic == buy_magic_num]
    # sell_positions = [pos for pos in positions if pos.magic == sell_magic_num]

    # buy_pos1211 = [pos for pos in positions if pos.magic == buy_magic_num and pos.volume in [opposite_qty10, opposite_qty11]]
    # sell_pos1211 = [pos for pos in positions if pos.magic == sell_magic_num and pos.volume in [opposite_qty10, opposite_qty11]]

    # # Calculate net profit for buy and sell strategies
    # net_buy_profit = sum(position.profit for position in buy_positions)
    # net_sell_profit = sum(position.profit for position in sell_positions)

    # # Get the count of buy and sell positions
    # buy_order_index = len(buy_positions)
    # sell_order_index = len(sell_positions)

    # buy_pos1211_index = len(buy_pos1211)
    # sell_pos1211_index = len(sell_pos1211)

    # # Define thresholds for orders and net profit
    # conditions = [
    #     (10, 10),       # Order 8: Close if profit >= 0
    # ]

    # for index_threshold, profit_threshold in conditions:
    #     if buy_order_index >= index_threshold and net_buy_profit >= profit_threshold:
    #         print(f"Closing buy strategy orders. Total Buy Profit: {net_buy_profit}")
    #         close_strategy_orders(symbol, buy_magic_num)
    #         break

    # for index_threshold, profit_threshold in conditions:
    #     if sell_order_index >= index_threshold and net_sell_profit >= profit_threshold:
    #         print(f"Closing sell strategy orders. Total Sell Profit: {net_sell_profit}")
    #         close_strategy_orders(symbol, sell_magic_num)
    #         break

    # # Check conditions for buy positions
    # for index_threshold, profit_threshold in conditions:
    #     if buy_pos1211_index == 2:
    #         if buy_order_index >= index_threshold and net_buy_profit >= 10:
    #             print(f"Closing buy strategy orders. Total Buy Profit: {net_buy_profit}")
    #             close_strategy_orders(symbol, buy_magic_num)
    #             break
    #     else:
    ...
    # for index_threshold, profit_threshold in conditions:
    #     if sell_pos1211_index == 2:
    #         if sell_order_index >= index_threshold and net_sell_profit >= 10:
    #             print(f"Closing sell strategy orders. Total Sell Profit: {net_sell_profit}")
    #             close_strategy_orders(symbol, sell_magic_num)
    #             break
    #         else:
...
def hedging_buy_update_sl_13(symbol):
    ...
    price_thresholds_13 = {
        ...
        change_pips_for_sl_2000: 1800,
        change_pips_for_sl_3000: 2800
        }
    ...
    buy_positions = [pos for pos in positions if pos.type == mt5.ORDER_TYPE_BUY 
                     and pos.magic == sell_magic_num and pos.volume in (opposite_qty14, opposite_qty16, opposite_qty05)]
...
def hedging_sell_update_sl_13(symbol):
    ...
    sell_positions = [pos for pos in positions if pos.type == mt5.ORDER_TYPE_SELL 
                      and pos.magic == buy_magic_num and pos.volume in (opposite_qty14, opposite_qty16, opposite_qty05)]
...
def close_profitable_small_orders(symbol):
    ...
    buy_orders = [pos for pos in positions if pos.magic == 1001 and pos.type == mt5.ORDER_TYPE_BUY]
    sell_orders = [pos for pos in positions if pos.magic == 2001 and pos.type == mt5.ORDER_TYPE_SELL]

    buy_small_orders = [pos for pos in positions if pos.magic == 1001 and pos.volume == 0.01]
    sell_small_orders = [pos for pos in positions if pos.magic == 2001 and pos.volume == 0.01]

    buy_two_orders = [pos for pos in positions if pos.magic == 1001 and pos.volume == 0.02]
    sell_two_orders = [pos for pos in positions if pos.magic == 2001 and pos.volume == 0.02]

    hedge_orders_buys = [pos for pos in positions if pos.magic == 2001 and pos.volume == 0.05]
    hedge_orders_sell = [pos for pos in positions if pos.magic == 1001 and pos.volume == 0.05]

    total_buys = len(buy_small_orders)
    total_sells = len(sell_small_orders)

    if (total_buys - total_sells) > 1:
        handle_order_closing(symbol, buy_small_orders)
    if (total_sells - total_buys) > 1:
        handle_order_closing(symbol, sell_small_orders)

    if (total_buys - total_sells) >= 0:
        handle_order_closing1(symbol, buy_small_orders)
    if (total_sells - total_buys) >= 0:
...
def handle_order_closing(symbol, small_orders):
    ...
    if latest_bors_order.profit > 2:  # If the latest hedge order is in loss
...
#     if latest_bors_order.profit > 10:  # If the latest hedge order is in loss
#         close_order(latest_bors_order, symbol)
...
def place_general_orders_buy_sell(symbol):
    ...
    if not buy_positions:
        ...
        if buy_price <= latest_buy_position_price - pips_diff * mt5.symbol_info(symbol).point:
    ...
    if not sell_positions:
        ...
        if sell_price >= latest_sell_position_price + pips_diff * mt5.symbol_info(symbol).point:
