...
def place_buy_order(symbol, buy_order_qty, magic_num):
    ...
    sl = price - pips_500 * mt5.symbol_info(symbol).point
...
def close_orders_based_on_conditions(symbol, price_15m, upper_band_15m, lower_band_15m, middle_band_15m, allow_buy_orders, allow_sell_orders, m5_trend, m15_trend, h1_trend):
...
# scenairo 1
    ...
    if martingale_sell_positions and len(martingale_sell_positions) >= 2:
    ...
    if martingale_buy_positions and len(martingale_buy_positions) >= 2:
...
# scenario 4 - min loss -- begins
    ...
    martingale_buy_pos = [bpos for bpos in positions if bpos.type == mt5.ORDER_TYPE_BUY and bpos.magic == martingale_buy_magic]
    martingale_sell_pos = [spos for spos in positions if spos.type == mt5.ORDER_TYPE_SELL and spos.magic == martingale_sell_magic]
...
# ========= GET MACD TRENDS DATA ========== 
    ...
    print(f"M5: {m5_trend} ({m5_macd:.5f}), M15: {m15_trend} ({m15_macd:.5f}), H1: {h1_trend} ({h1_macd:.5f}), Rev: {h1_reversal}, Type: {reversal_type}, RSI: {rsi}, buy: {buy_now_confirmed}, sell: {sell_now_confirmed}, bb_gap: {bb_gap}, direction: {direction}")
    ...
    close_orders_based_on_conditions(symbol, price_15m, upper_band_15m, lower_band_15m, middle_band_15m, allow_buy_orders, allow_sell_orders, m5_trend, m15_trend, h1_trend)
    ...
    if not martingale_buy_positions and not martingale_sell_positions and not trend_buy_positions and not trend_sell_positions:
        if not is_trading_allowed_uk(symbol):
            # print(f"no trading - {mt5_server_time(symbol)}")
            return
        # else:
        #     print(f"trading started now - {mt5_server_time(symbol)}")

    # if is_gold_trading_allowed(symbol):
    if not martingale_buy_positions:    
        ...
        # if buy_price <= round(middle_band_15m - bb15m_tolerance,2):  # Initial buy condition
        if buy_price <= lower_band_15m + 1:
            if (m5_trend == "SELL" and m15_trend == "SELL" and h1_trend == "SELL") or direction == "DOWN" or rsi > sell_rsi:
                pass
            else:
    ...
    if not martingale_sell_positions:
        ...
        # if sell_price >= round(middle_band_15m + bb15m_tolerance,2):  # Initial sell condition
        if sell_price >= upper_band_15m - 1:  # Initial sell condition
            if (m5_trend == "BUY" and m15_trend == "BUY" and h1_trend == "BUY") or  direction == "UP" or rsi < buy_rsi:
                pass
            else:
    ...
    #     martingale_buy_order_qty = martingale_lot_sizes[0]
    #     if allow_buy_orders and m5_trend == "BUY" and rsi < 70 and bb_gap > 30:  # Initial buy condition
    #         place_buy_order(symbol, martingale_buy_order_qty, trend_buy_magic)
    #         print(f"MACD Trends - M5: {m5_trend} ({m5_macd:.5f}), M15: {m15_trend} ({m15_macd:.5f}), H1: {h1_trend} ({h1_macd:.5f}) - H1 Reversal: {h1_reversal}, Type: {reversal_type}, RSI: {rsi}, Supp: {latest['active_support']}, Res: {latest['active_resistance']}")

    # if not trend_sell_positions:
    #     martingale_sell_order_qty = martingale_lot_sizes[0]
    #     if allow_sell_orders and m5_trend == "SELL" and rsi > 30 and bb_gap > 30:  # Initial sell condition
    #         place_sell_order(symbol, martingale_sell_order_qty, trend_sell_magic)
    #         print(f"MACD Trends - M5: {m5_trend} ({m5_macd:.5f}), M15: {m15_trend} ({m15_macd:.5f}), H1: {h1_trend} ({h1_macd:.5f}) - H1 Reversal: {h1_reversal}, Type: {reversal_type}, RSI: {rsi}, Supp: {latest['active_support']}, Res: {latest['active_resistance']}")

    # if not bb15m_buy_positions:    
    #     martingale_buy_order_qty = martingale_lot_sizes[0]
    #     if buy_price <= lower_band_15m + bb15m_lower_tolerance and not changing_to_bearish and bb_gap < 30 and buy_now_confirmed:  # Initial buy condition
    #         place_buy_order(symbol, martingale_buy_order_qty, bb_buy_magic)
    #         print(f"MACD Trends - M5: {m5_trend} ({m5_macd:.5f}), M15: {m15_trend} ({m15_macd:.5f}), H1: {h1_trend} ({h1_macd:.5f}) - H1 Reversal: {h1_reversal}, Type: {reversal_type}, RSI: {rsi}, Supp: {latest['active_support']}, Res: {latest['active_resistance']}")
    #         # print(f"Active Support: {latest['active_support']}, Active Resistance: {latest['active_resistance']}, latest bull wick: {latest['bull_wick']}, latest bear wick: {latest['bear_wick']}")
    # if not bb15m_sell_positions:
    #     martingale_sell_order_qty = martingale_lot_sizes[0]
    #     if sell_price >= upper_band_15m - bb15m_upper_tolerance and not changing_to_bullish and bb_gap < 30 and sell_now_confirmed:  # Initial sell condition
    #         place_sell_order(symbol, martingale_sell_order_qty, bb_sell_magic)
    #         print(f"MACD Trends - M5: {m5_trend} ({m5_macd:.5f}), M15: {m15_trend} ({m15_macd:.5f}), H1: {h1_trend} ({h1_macd:.5f}) - H1 Reversal: {h1_reversal}, Type: {reversal_type}, RSI: {rsi}, Supp: {latest['active_support']}, Res: {latest['active_resistance']}")
    #         # print(f"Active Support: {latest['active_support']}, Active Resistance: {latest['active_resistance']}, latest bull wick: {latest['bull_wick']}, latest bear wick: {latest['bear_wick']}")
