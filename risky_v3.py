...
# scenario 1 -- end
    ...
    if round(buy_total - sell_total, 2) > 0.10:
        if can_place_new_sell(sell_price, spl_sell_positions, buffer_points=5.0):
            place_sell_order(symbol, opposite_qty01, special_magic_num)

    if round(sell_total - buy_total, 2) > 0.10:
        if can_place_new_buy(buy_price, spl_buy_positions, buffer_points=5.0):
            place_buy_order(symbol, opposite_qty01, special_magic_num)
...
def can_place_new_buy(current_price, buy_positions, buffer_points=5.0):
    """
...
def can_place_new_sell(current_price, sell_positions, buffer_points=5.0):
