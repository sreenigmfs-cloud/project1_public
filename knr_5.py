...
lot_sizes = [0.01, 0.01, 0.01, 0.01, 0.01] 
opposite_size = 0.09
...
# scenario 1 -- end
    ...
    if total_positions >= 4 and total_profit <= -30:
        close_strategy_orders1(symbol)


    if total_positions >= 5 and total_profit >= 0:
        close_strategy_orders1(symbol)
...
def get_latest_buy_tp(symbol, magic_num):
    ...
    # Filter for buy positions
    buy_positions = [pos for pos in positions if pos.type == mt5.ORDER_TYPE_BUY and pos.volume == 0.01]
    if not buy_positions:
...
def get_latest_sell_tp(symbol, magic_num):
    ...
    # Filter for sell positions
    sell_positions = [pos for pos in positions if pos.type == mt5.ORDER_TYPE_SELL and pos.volume == 0.01]
    if not sell_positions:
        print("No buy positions found.")
        return None
...
def close_profitable_small_orders(symbol):
    ...
    buy_orders_small = [pos for pos in positions if pos.magic == 1001 and pos.volume in [0.01, 0.02]]
    sell_orders_small = [pos for pos in positions if pos.magic == 2001 and pos.volume in [0.01, 0.02]]
...
def handle_order_closing_big(symbol, small_orders):
    ...
    #     if pos.volume == 0.02 and pos.profit >= pos.volume * profit_amount:
...
def monitor_and_place_opposite_orders(symbol):
    ...
    if len(buy_only) == 3 and all(order.profit < -4 for order in buy_only):
        if len(buy_hedge_q09) < 1:        
            place_sell_order(symbol, opposite_size, magic_num=1001)

    if len(sell_only) == 3 and all(order.profit < -4 for order in sell_only):
        if len(sell_hedge_q09) < 1: 
